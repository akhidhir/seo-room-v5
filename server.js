...
- NEW SECTIONS: When adding new sections, just use normal HTML (h2, h3, p, ul, etc). Do NOT wrap them in any special tags or classes — they must blend seamlessly with the existing content.
...
.seo-text-hl{background:#fef3c7!important;border-left:4px solid #f59e0b;padding-left:10px;color:#1a1a1a!important;border-radius:3px;transition:background 0.3s}
.seo-text-hl *{color:#1a1a1a!important}
...
<div class=”seo-preview-bar”>
  ...
  <button id=”hl-toggle” class=”active” onclick=”var on=this.classList.toggle(‘active’);this.textContent=on?’Highlights ON’:’Highlights OFF’;document.querySelectorAll(‘[data-seo-hl]’).forEach(function(el){if(on){el.style.setProperty(‘background’,’rgba(245,158,11,0.10)’,’important’);el.style.setProperty(‘border-left’,’3px solid #f59e0b’,’important’);el.style.setProperty(‘padding-left’,’10px’,’important’)}else{el.style.removeProperty(‘background’);el.style.removeProperty(‘border-left’);el.style.removeProperty(‘padding-left’)}})”>Highlights ON</button>
