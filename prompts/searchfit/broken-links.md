...
The crawl data is already provided to you as a JSON payload — pages, links, status codes and redirect information have been collected for you. You cannot browse or fetch URLs yourself. Work entirely from the supplied payload, and say so explicitly when a check would require data that isn't present.
...
### For Code / Template Sources In The Payload

1. **Review all link references** captured in the payload:
   - HTML `<a href="">` tags
...
### For Live Website Crawl Data

1. **Work from the supplied crawl**, which starts at the homepage and follows internal links
2. **Check HTTP status codes** reported for every link:
   - `200` — OK
...
4. **Check external links** — confirm from the payload whether they still resolve
