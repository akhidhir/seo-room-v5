...
Infer the following from the crawl data and project context provided. Do not ask the user questions — state your assumptions where the data is thin:
...
5. **Desired word count**: Or recommend one based on competitor data in the payload

### Step 2: Analyze the SERP

Use whatever competitor and SERP data is included in the payload — you cannot browse the web yourself. From that data, determine:
...
If no SERP data is supplied, say so and base the brief on the site's own content and the project context.
