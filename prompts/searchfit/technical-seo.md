...
The crawl data is already provided to you as a JSON payload — robots.txt, sitemap, status codes, headers, page HTML, schema and any performance metrics have been collected for you. You cannot browse, crawl, or run live tests yourself. Audit only what is in the payload, and explicitly note any check you cannot perform because the data isn't there.
...
### If the payload contains source/config files
...
1. Analyze the supplied robots.txt and sitemap
2. Check the HTTP headers and status codes captured in the payload
3. Analyze the page load performance metrics supplied
4. Check mobile rendering signals present in the payload
5. Validate structured data
6. Review key user journeys for technical issues visible in the data
